...
/// CSV export for log cell data.
/// Ported from src/lib/export.ts → logCellsToCSV()
enum CSVExporter {

    /// Comma-separated column header row for exported log cell CSV files.
    ///
    /// Columns cover cell metadata (number, timestamp, flags), environmental readings
    /// (temperature, pressure, humidity, CO2, PM, VOC, NOx), and Stroop task results.
    static let header = [
    ...
    /// Generate CSV string from LogCellEntity array.
    ...
    /// Write CSV to a temporary file and return its URL for sharing.
